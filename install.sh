...
V2BX_FOUND=false
V2BX_TYPE=""  # systemd / docker / binary
    ...
    case "$V2BX_ACTION" in
        ...
        2)
            ...
            if [[ "$V2BX_TYPE" == "systemd" ]]; then
                systemctl stop "$V2BX_SERVICE" 2>/dev/null || true
                systemctl disable "$V2BX_SERVICE" 2>/dev/null || true
                info "已停止 systemd 服务: $V2BX_SERVICE (已禁用开机自启)"
            fi
            if [[ "$V2BX_TYPE" == "docker" ]]; then
                docker stop "$V2BX_CONTAINER" 2>/dev/null || true
                info "已停止 Docker 容器: $V2BX_CONTAINER"
            fi
            info "V2bX 已停止，文件保留。恢复方式:"
            if [[ "$V2BX_TYPE" == "systemd" ]]; then
                echo "  systemctl enable --now $V2BX_SERVICE"
            elif [[ "$V2BX_TYPE" == "docker" ]]; then
                echo "  docker start $V2BX_CONTAINER"
            fi
            ;;
        3)
            warn "跳过 V2bX 处理，如果端口冲突请手动解决"
            ;;
        0)
            info "退出安装"
            exit 0
...
# ══════════════════════════════════════════════════════════════════════════════
# 3. 处理有问题的仓库源 + 安装 Docker & Git
# ══════════════════════════════════════════════════════════════════════════════
step "3/6  安装依赖 (Docker & Git)"
